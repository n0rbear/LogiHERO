...
- `POST /api/sync-tours/:driverName` (`ImportEngine.processTour`) — the route Android's tour/stop lifecycle sync actually uses — did not re-verify pending cargo pickup/delivery blocking server-side before accepting a client-supplied `stop_status = COMPLETED`; only the Android client checked it. Reproduced against main `6a32d07` and fixed on branch `claude/legacy-tour-sync-cargo-blocking` (not yet merged): the cargo-blocking rule now lives in `src/engines/cargo-lifecycle.js` and is shared by the dedicated stop-complete endpoint and the legacy bulk sync. Adversarial review of that first fix then found it could be satisfied with forged cargo state supplied in the same payload, so the same branch also made the server authoritative over cargo in mobile sync: only cargo already on the synced tour is addressable, only its status is client-writable, and only along a transition the dedicated driver endpoints allow. Cargo creation, deletion, stop/tour re-pointing and cross-tour reassignment through mobile sync are now refused, and accepted offline transitions leave a `cargo_events` audit row. See TD-009.
- `ImportEngine.processTour` resolves the tour by client-supplied UUID with no driver/company scope, so a driver who knows another driver's tour UUID can have their sync payload applied against that tour. Pre-existing, reproduced against real PostgreSQL, and narrowed but not closed by the PR #5 cargo work. See TD-010.
