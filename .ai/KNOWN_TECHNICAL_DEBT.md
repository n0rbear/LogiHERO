...
A follow-up 2026-09-06 domain invariant audit of generic `/api/sync` found and closed six confirmed non-ownership bypasses (cost approval/payment status, unauthorized cargo creation and status/deletion, hotel status/deletion, writes to admin-approved work days/entries, and tour/stop completion-state forgery). TD-004 is now closed for generic `/api/sync`. That same audit traced actual Android source and found the real production tour/stop completion path is a *different*, pre-existing route (`POST /api/sync-tours/:driverName`) which has its own narrower, separate gap — see TD-009.
...
Risk: low — closed with evidence, not inference.
