...
Remaining evidence: `tests/legacy-tour-sync-cargo-blocking.test.js` (6 tests) covers both bypass variants (each confirmed to fail against `6a32d07`), satisfied cargo, terminal cargo, the legitimate offline pickup+complete batch, and the already-completed re-send case. Not yet verified against production data. Note this route also lets a mobile payload set cargo `status` directly, which is a separate question from stop completion and was not addressed here.
